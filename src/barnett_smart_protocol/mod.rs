...
use proof_base::vector_commitment::pedersen::PedersenCommitment;
use proof_base::vector_commitment::{pedersen, HomomorphicCommitmentScheme};
use proof_base::zkp::{
    arguments::shuffle,
...
pub trait BarnettSmartProtocol {
    ...
    fn setup<R: Rng>(
        ...
        m: usize,
        n: usize,
    ) -> std::result::Result<Self::Parameters, CardProtocolError>;

    /// Generate keys for a player.
    fn player_keygen<R: Rng>(
        rng: &mut R,
        pp: &Self::Parameters,
    ) -> std::result::Result<(Self::PlayerPublicKey, Self::PlayerSecretKey), CardProtocolError>;
    ...
    ) -> std::result::Result<Self::ZKProofKeyOwnership, CryptoError>;
    ...
    ) -> std::result::Result<Self::AggregatePublicKey, CardProtocolError>;
    ...
    fn mask<R: Rng>(
        ...
        alpha: &Self::Scalar,
    ) -> std::result::Result<(Self::MaskedCard, Self::ZKProofMasking), CardProtocolError>;
    ...
    fn remask<R: Rng>(
        ...
        alpha: &Self::Scalar,
    ) -> std::result::Result<(Self::MaskedCard, Self::ZKProofRemasking), CardProtocolError>;
    ...
    fn compute_reveal_token<R: Rng>(
        ...
        pk: &Self::PlayerPublicKey,
        masked_card: &Self::MaskedCard,
    ) -> std::result::Result<(Self::RevealToken, Self::ZKProofReveal), CardProtocolError>;
    ...
    fn unmask(
        ...
        masked_card: &Self::MaskedCard,
    ) -> std::result::Result<Self::Card, CardProtocolError>;
    ...
    ) -> std::result::Result<(Vec<Self::MaskedCard>, Self::ZKProofShuffle), CardProtocolError>;
...
impl<C: CurveGroup> BarnettSmartProtocol for DLCards<C> {
    ...
    fn mask<R: Rng>(
        ...
        let generator = pp.enc_parameters.generator;

        // Map to Chaum-Pedersen parameters
        let cp_parameters = chaum_pedersen_dl_equality::Parameters::new(&generator, shared_key);

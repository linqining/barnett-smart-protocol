...
mod test {
    ...
    fn setup_players<R: Rng>(
        ...
        let (pk0, sk0) = CardProtocol::player_keygen(rng, &parameters).unwrap();
        let info0 = Scalar::rand(rng);
        players.push((pk0, sk0, info0));
        let mut expected_shared_key = players[0].0;
        for _ in 1..num_of_players {
            ...
            expected_shared_key = (expected_shared_key + pk).into();
        }
    ...
    fn aggregate_keys() {
        ...
        bad_key_proof_pairs[0].0 = PublicKey::identity();
